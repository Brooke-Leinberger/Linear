...
*/
vector* create_vector( int count, const double* elements, vector* dest )
{
    //A vector must have a positive amount of elements
	if( count < 1 )
		return NULL;

    //If the destination is empty, allocate a new vector
	if( !dest )
	{
    ...
    //if dest is not empty, and has a differing amount of elements, fail
	else if(dest->count != count)
			return NULL;

    //if elements is NULL, initialize the vectors with zeros
	if( !elements )
		for( int i = 0; i < count; i++ )
            dest->elements[i] = 0;

    //otherwise, copy in the provided elements
	else
		for( int i = 0; i < count; i++ )
...
*/
double* read_vector( vector*  vec, double *dest )
{
	if( !vec )
		return NULL;

	if( !dest )
		dest = malloc(vec->count * sizeof(double));

	for( int i = 0; i < vec->count; i++ )
		dest[i] = vec->elements[i];
...
{
	//NULL and compatibility checks are done in scale_vector
...
/*
 * Returns angular distance between two vectors in radians
 * Returns -10 if a, b, or up are NULL
 * Domain of returned values range from 0 to PI
 * */
double angular_distance (vector *a, vector *b)
{
        if( !a || !b )
                return -10;

        return acos( dot_product(a,b) / (magnitude_vector(a) * magnitude_vector(b)));
}

/*
 * Returns angular displacement between two vectors in radians
 * Returns -10 if a, b, or up are NULL
 * Domain of returned values range from -PI to PI
 * */
double angular_displacement (vector *a, vector *b, vector *up)
{
        if( !a || !b || !up )
                return -10;

        vector *cross = cross_product(a, b, NULL);
        double result = atan2( dot_product(cross,up) , dot_product(a, b) );
        free_vector(cross);
        return result;
}
